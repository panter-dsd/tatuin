...
use crate::ui::{keyboard_handler::KeyboardHandler, mouse_handler::MouseHandler, style, widgets::MarkdownLineConfig};
...
impl TaskRow {
    pub fn new(t: &dyn TaskTrait, changed_tasks: &[TaskPatch]) -> Self {
        ...
        let mut children: Vec<Box<dyn WidgetTrait>> = vec![
            ...
            Box::new(
                MarkdownLine::new(name.as_str(), MarkdownLineConfig::default())
